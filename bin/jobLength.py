...
def run():
    ...
    for thisSecond in range(startTime, stopTime+1):
       x = 0
       for ent in entries:
            if thisSecond == ent.executionStopTime:
                timeItTook = ent.executionStopTime - ent.executionStartTime
                print datetime.datetime.fromtimestamp(thisSecond).strftime('%H:%M:%S')+" "+str(timeItTook)
